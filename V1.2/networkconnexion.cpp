...
void NetworkConnexion::managerSslError(QNetworkReply *reply, const QList<QSslError> &errors)
{
    ...
    {
        nbr.setNum(i+1);
        errorMessage += tr("Erreur ") + nbr + " : " + errors.at(i).errorString() + "\n";
    }
    emit showErrorMessage(tr("Erreur SSL"), errorMessage);
}

void NetworkConnexion::replyNetworkError(QNetworkReply::NetworkError code)
{
    emit showErrorMessage(tr("Erreur réseau"), tr("Erreur : ") + code);
}
